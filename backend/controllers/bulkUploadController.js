...
exports.bulkUpload = async (req, res) => {
  ...
  // ── Extract ZIP images into temp map: slug → [{ filename, buffer }] ──────
  const imageMap = {}; // { slug: [{ ext, buffer }] }
  if (zipFile) {
    try {
      ...
      zip.getEntries().forEach(entry => {
        ...
        // filename format: slug.jpg OR slug-1.jpg OR slug-2.jpg
    ...
    try {
      ...
      if (productImages.length) {
        ...
        productImages.forEach((img, idx) => {
          const filename = `${idx + 1}${img.ext}`;
          fs.writeFileSync(path.join(productDir, filename), img.buffer);
      ...
      if (productImages.length) {
        ...
        for (let i = 0; i < productImages.length; i++) {
          const imgPath = `/uploads/${slug}/${i+1}${productImages[i].ext}`;
          await db.query(
