...
export default function BulkUpload() {
  ...
  const downloadTemplate = () => {
    const headers = ['name','slug','category','price','mrp','stock','composition','prescription','description','manufacturer','expiry'];
    const sample  = ['Agoc-SP Tablets','agoc-sp-tablets','Tablets',350,420,100,'Aceclofenac 100mg + Paracetamol 325mg','Yes','Used for pain and inflammation','Agoc Care Pvt. Ltd.','12/2026'];
    const csv = [headers.join(','), sample.join(',')].join('\n');
    const a = Object.assign(document.createElement('a'), {
      href: URL.createObjectURL(new Blob([csv], { type: 'text/csv' })),
      download: 'AgocCare_Bulk_Upload_Template.csv'
    });
    a.click();
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-secondary/10 border border-secondary/20 rounded-2xl p-5">
        <h2 className="font-bold text-teal mb-3">How to use</h2>
        <ol className="space-y-1.5 text-sm text-gray-600 list-decimal list-inside">
          <li>Download the Excel template and fill in product details</li>
          <li>Name images as <code className="bg-gray-100 px-1 rounded text-xs">product-slug.jpg</code> or <code className="bg-gray-100 px-1 rounded text-xs">product-slug-1.jpg</code> for multiple</li>
          <li>ZIP all images into a single file</li>
          <li>Upload both Excel and ZIP below</li>
        </ol>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-teal-mid/30 shadow-card p-6 space-y-5">
        ...
        {/* Excel */}
        <div>
          <label className="label">Excel / CSV File * <span className="text-gray-400 normal-case font-normal">(required)</span></label>
          <div onClick={() => excelRef.current?.click()}
            ...
            {excel ? (
              <div className="flex items-center justify-center gap-2 text-cta-dark">
                ...
                <span className="text-xs text-gray-400">({(excel.size/1024).toFixed(1)} KB)</span>
              </div>
            ) : (
              <div>
        ...
        {/* ZIP */}
        <div>
          <label className="label">Images ZIP File <span className="text-gray-400 normal-case font-normal">(optional — max 100MB)</span></label>
          <div onClick={() => zipRef.current?.click()}
            ...
            {zip ? (
              <div className="flex items-center justify-center gap-2 text-secondary">
                ...
                <span className="text-xs text-gray-400">({(zip.size/1024/1024).toFixed(2)} MB)</span>
              </div>
            ) : (
              <div>
                <svg className="w-10 h-10 text-gray-300 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <p className="text-xs text-gray-400 mt-1">.zip format only</p>
              </div>
            )}
          </div>
        </div>
        ...
        {loading && (
          <div className="space-y-2">
            <div className="flex justify-between text-xs text-gray-500">
              <span>Uploading and processing...</span><span>{progress}%</span>
            </div>
        ...
        <motion.button type="submit" disabled={loading || !excel}
          ...
          {loading
            ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"/>Processing... Please wait</span>
            ...
            {result.success.length > 0 && (
              <div className="bg-white rounded-2xl border border-green-100 shadow-card overflow-hidden">
                <div className="bg-green-50 px-5 py-3 border-b border-green-100">
                  <h3 className="font-bold text-green-700">✅ Successfully Imported ({result.success.length})</h3>
                </div>
            ...
            {result.failed.length > 0 && (
              <div className="bg-white rounded-2xl border border-red-100 shadow-card overflow-hidden">
                <div className="bg-red-50 px-5 py-3 border-b border-red-100">
                  <h3 className="font-bold text-red-600">❌ Failed ({result.failed.length})</h3>
                </div>
