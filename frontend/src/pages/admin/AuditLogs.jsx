        ...
        {Object.values(filters).some(v => v) && (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {Object.entries(filters).filter(([,v]) => v).map(([k, v]) => (
              <span key={k} className="bg-primary-light text-primary text-xs font-semibold px-2 py-0.5 rounded-lg">
                {k}: {v}
              </span>
            ))}
                  ...
                  <motion.tr key={log.id}
                    ...
                    <td className="px-4 py-3.5">
                      {(log.old_value || log.new_value) && (
                        <button className={`text-xs font-bold transition-colors ${expanded === log.id ? 'text-primary' : 'text-gray-400 hover:text-primary'}`}>
            ...
            <div className="flex gap-1">
              ...
              {Array.from({ length: Math.min(5, pages) }, (_, i) => i + Math.max(1, page-2)).filter(p => p <= pages).map(p => (
                <button key={p} onClick={() => setPage(p)}
                  className={`w-8 h-8 rounded-lg text-sm font-bold transition-colors ${page===p ? 'bg-primary text-white' : 'text-gray-500 hover:bg-gray-100'}`}>
                  {p}
                </button>
              ))}
