...
export default function Prescription() {
  ...
  const handleFile = (f) => {
    ...
    toast.success('Prescription ready! Click Extract to scan.');
  };
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      setResult(data);
      const count = data.matched_products?.filter(g => g.products.length > 0).length || 0;
      if (data.medicines?.length === 0) {
        toast.error('No medicines found. Try a clearer image.');
      } else {
        toast.success(`Found ${data.medicines.length} medicine(s) — ${count} matched in store!`);
      }
    } catch (err) {
  ...
  return (
    <>
      <SeoHead pageKey="prescription" defaults={{
        title: 'Upload Prescription – AgocCare',
        description: "Upload your prescription and we'll scan it to find matching medicines automatically.",
      }} />
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <motion.div className="text-center mb-10"
            ...
            <motion.p variants={fadeUp} className="text-gray-500 max-w-md mx-auto text-sm leading-relaxed">
              Upload your doctor's prescription — we'll scan and find matching medicines from our store instantly.
            </motion.p>
          </motion.div>
          ...
          <motion.div className="grid grid-cols-3 gap-4 mb-8"
            variants={staggerContainer(0.1)} initial="hidden" animate="visible">
            {[
              { num:'01', title:'Upload Prescription', sub:'Photo of your prescription', icon:'📄' },
              { num:'02', title:'Scan & Extract',      sub:'Text is read automatically', icon:'🔍' },
              { num:'03', title:'Add to Cart',         sub:'Order matched medicines',    icon:'🛒' },
            ].map(s => (
              ...
              {!file && (
                <div className="py-16 flex flex-col items-center gap-3">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-400 mt-0.5">or click to browse · JPG or PNG · Max 10MB</p>
                  </div>
                </div>
              )}
                ...
                <div className="p-5">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400">{(file.size / 1024).toFixed(1)} KB · Ready to scan</p>
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible" className="text-center mb-8">
            ...
            <p className="text-xs text-gray-400 mt-2">Works offline — no AI service required</p>
          </motion.div>
                ...
                {result.medicines?.length > 0 && (
                  <div className="bg-white rounded-2xl border border-teal-mid/30 p-5 mb-6 shadow-sm">
                    <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                      ...
                      <motion.button onClick={handleAddAll}
                    ...
                    {result.matched_products.map((group, gi) => (
                      <motion.div key={gi}
                        ...
                        <div className="bg-teal-light px-5 py-3 flex items-center gap-2 border-b border-teal-mid/30">
                          ...
                          <span className="text-sm font-bold text-teal">{group.extracted.name}</span>
                ...
                {result.medicines?.length === 0 && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    ...
                    <p className="text-sm text-gray-400 mb-4">
                      Could not read the prescription clearly. Please try:<br/>
                      · A clearer, well-lit photo · Make sure text is visible · Avoid shadows or blur
                    </p>
          ...
          <motion.p variants={fadeUp} initial="hidden" animate="visible"
            ...
            ⚕️ Always consult your doctor before purchasing prescription medicines. Prescription medicines (Rx) require a valid prescription.
          </motion.p>
        </div>
      </div>
    </>
